...
5.  **Dashboard 纯只读镜子（0.11.0 新增）：** `/cc-code:dashboard` 拉起的页面数字永不作为回答用户问题的依据，唯一真相源永远是 `active/*.md` + git。`.cc_code/.runtime/`（pid / 端口 / dashboard 派发用的固定 session id）是运行时产物，AI 禁止读写，不算规范 8 文件之一。
