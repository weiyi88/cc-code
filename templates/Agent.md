...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| F-3 | 2026-09-03 | L1 | prd.md | 登录 | 新增会话过期规则 A12 | 共存 | — |
...
```
   ...
   最纯净 ── 每一行都在回答「现在是什么」, 不是「当时怎么决定的」
             过程产物不落盘（对话内裁决, 留痕 = change-log 一行）
             逐轮验收详情 → docs/qa/ ; 历史版本靠 git (.cc_code 在版本控制内, 不另存快照)
...
*   **文件权限：**
    ...
    *   `[可写]` `active/project.md`, `active/data.md`, `active/api.md`（阶段拆分并入 project.md 章节）
    *   `[禁读]` `src/` 下的具体业务代码
...
5.  **输出可读（铁律）：** 任何输出给用户的断言引用一律双标识「编号｜中文模块-功能」（如 `A12｜登录-会话过期登出 → ❌ FAIL`、`U2.1｜看板页-卡片拖拽 → ✅`、`B-3｜登录-重复提交 401 → ✅`）。⛔ 禁止只甩裸编号（`A12`）或裸代号（`M1`）—— 用户不该为看懂结果去查表。
    > 本条**全员生效**：不限 QA，凡向用户输出断言引用的主体（各角色 AI、执行编排器主控、全量清算与诊断报告）一律适用。⛔ 各 skill 正文不得复述本条——规则单一出处在此。
