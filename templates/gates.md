...
| **只留未关闭** | §三/§四/§五 只留**未修复**项，修完即移出（历史在 `docs/qa/` + `back_up/change-log.md`） |
| **历史靠 git** | 旧版本不在本文件留存，`git log -p` 即完整历史 |

> 📢 **输出可读铁律**：任何输出给用户的断言引用一律双标识「编号｜中文模块-功能」（如 `A12｜登录-会话过期登出 → ❌ FAIL`），⛔ 禁止只甩裸编号或 `M1` 这类代号。
...
### 2.3 B 段 — bug 修复（尺子：`bugs.md` 的 B-n 条目「期望出处」）

> 仅记录**期望出处为主人原话**（无既有 A/U 断言对应）的 B-n 结果；期望出处为既有 A/U 断言的 bug **就地改写 2.1 / 2.2 对应行**，不在此开行。
> 行生命周期：修复 PASS 后此行**永久保留**（追溯用）；`bugs.md` 条目才删除。

| B 编号 | Verdict | 日期 | 回归测试位置 | 实测结果 |
| :--- | :--- | :--- | :--- | :--- |
| B-1 | ✅ PASS / ❌ FAIL | [日期] | `[测试文件 + 用例名]` | [待填写] |
...
> 🗄️ 逐轮完整报告在 `docs/qa/<YYYY-MM-DD>-round-N.md`（温数据）；跨项目时间线归档在 `back_up/change-log.md`（冷数据）。本文件不留任何轮次索引/附录。
