...
| 5 | 已作废/已删功能 | `active/prd.md` 历史增量的冲突收敛表 | L1 |
...
| 层 | 规范文件 | 唯一写者 | 本 skill 落什么 | 绝不写 |
| --- | --- | --- | --- | --- |
| **L1** | `active/prd.md` | PM | 需求清单 R · 验收断言 **A 序列续编** · 业务状态机 · 冲突收敛表 · 待澄清 · Out of Scope 追加 | 线框、字段类型、接口参数 |
| **L2** | `active/ux.md` | PM | 视觉规格（原型**目标态**线框）· 交互五态矩阵 · 组件清单 | 业务规则、字段类型 |
| **L3** | `active/project.md` | Architect | 架构决策 · 技术选型 · 目录 · 任务拆分 | 需求、线框 |
| **L3** | `active/data.md` | Architect | 数据契约变更 · 迁移规则 · 索引 | 需求、线框 |
| **L3** | `active/api.md` | Architect | method / path / 入参 / 出参 / 错误码 | 需求、线框 |
...
**判据速查：** 能脱离界面存在的 → L1；离开界面就没意义的 → L2；只有工程师关心的 → L3。

**增量批次号：** 读 L1/L2/L3 各文件已有最大 `F-n` → 本次取 `F-(n+1)`。同一需求在**所有命中文件用同一个 F 号**，便于跨文件追溯。章节标题统一为：

```
## 增量 F-3 — <需求名>（YYYY-MM-DD）
```
...
| 三件套 | 落哪 | 说明 |
| --- | --- | --- |
| ① 逻辑图 · 业务状态机 | L1 `prd.md` | 业务流转 |
| ① 逻辑图 · 页面流转 | L2 `ux.md` | 交互五态 |
| ② 原型双联 · **目标态** | L2 `ux.md` | 只落目标态 |
| ② 原型双联 · **现状态** | **不落盘** | 仅服务 plan 内交谈与裁决，避免 `ux.md` 堆历史 |
| ③ 差异表 | L1 `prd.md` 冲突收敛表 | 现状→目标的差异在此留档 |
...
8.2 for 每个批次：
     ...
     ├─ 逐文件追加「## 增量 F-n — <需求名>（YYYY-MM-DD）」章节
     └─ 报本批产物清单
8.3 落完报「Dev / QA 域未动」，提示主人走 /cc-code:agent-to-mvp
```
...
| 契约漂移 | 不涉及 | 撞出即停，逐条二选一 |
| 产出 | 覆写 `prd.md`（旧版归档 `backup/`） | **追加**增量章节到命中层文件，不覆写 |
| 落盘层 | 仅 L1 | L1 + L2 + L3 动态命中 |
| 角色 | 单一 PM 域 | PM 批 → Architect 批，逐批请示切换 |
| 断言 | A1..An 全新 | A 序列续编，作废加删除线 |
