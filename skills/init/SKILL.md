...
| `prd.md` | §1.5 验收断言**主表** · 写入纪律段 · 文末变更台账 | 断言散落各处，QA 拿不全尺子 |
| `ux.md` | §2.3 `U` 编号五态矩阵 · §2.4 元素清单（含序号 / 适用态列）· 写入纪律段 · 文末变更台账 | **UI/五态无判定编号 → gates 无法逐条溯源，覆盖率是黑洞** |
| `gates.md` | §二 验收追溯矩阵（`A` 段 + `U` 段）· §六 四分母覆盖率 · 附录轮次归档索引 · 写入纪律段 | **退化成逐轮流水日志，「某断言现在什么状态」要跨 N 轮 grep** |
| `api.md` | 写入纪律段 · 文末变更台账 | 同一 path 散成 N 段补丁，Dev 得脑内拼接 |
| `data.md` | 写入纪律段 · 文末变更台账 | 同一张表散成主体 + N 个扩展段 |
| `project.md` | 写入纪律段 · 文末变更台账 | 架构决策堆积历史 |
| `status.md` | 当前坐标 · 下一步 · 里程碑（≤10 条） | 坐标丢失，AI 发散 |
...
| 发现的旧形态 | 归位动作 |
| --- | --- |
| 任意 active 文件有 `## 增量 F-n` 章节 | 章节内的**当前态规格**就地合并进对应小节（同 interface / 同 path / 同模块 / 同页面合成一段）；**过程性内容**（需求清单 / 冲突收敛表 / 漂移登记 / 迁移清单）迁 `docs/plans/F-n-<需求名>.md`；文末台账补一行 |
| `prd.md` 断言散在多个增量小节 | 全部**并回 §1.5 主表**（编号原样，禁重排）；原小节的过程内容迁 `docs/plans/` |
| `gates.md` 有「第 N 轮回归 / 第 N 轮验收」章节 | 各轮详情迁 `docs/qa/<日期>-round-N.md`；结果**汇总进 §二 追溯矩阵**（每断言一行，取最新一轮结论）；附录归档索引逐轮补一行 |
