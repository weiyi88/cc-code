...
脚本幂等：已存在 `.cc_code/` 则仅执行散落物迁移（不碰 active/，保护已填业务数据）。生成 `active/ backup/ docs/{plans,qa}/ images/ scripts/` + **8 个 active 模板骨架**（按 L0~L4 分层：`Agent status` / `prd` / `ux` / `project data api` / `gates`）+ **根目录 `CLAUDE.md` 入口引导**。
