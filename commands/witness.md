...
1. `$CLAUDE_PLUGIN_ROOT/skills/witness/engine.md`（脚本生成引擎）
2. `$CLAUDE_PLUGIN_ROOT/skills/witness/protocol/research.md`（史料查证协议）
3. `$CLAUDE_PLUGIN_ROOT/skills/witness/protocol/script-format.md`（脚本格式规范）
4. `$CLAUDE_PLUGIN_ROOT/skills/witness/protocol/annotation.md`（虚实标注规范）

### 第二步：匹配事件

读取 `$CLAUDE_PLUGIN_ROOT/skills/witness/events/catalog.yaml`，用模糊匹配找到用户请求的历史事件。
- 匹配规则：事件名、别名、关键词、年份均可匹配
- 未匹配到时，用 Exa 搜索确认是否为有效历史事件，再动态添加

### 第三步：查证史实

按 `protocol/research.md` 执行史料查证，获取：
- 事件时间线（精确到日）
- 核心人物及其立场
- 关键地点与场景
- 争议点与不同史学观点
...
按 `engine.md` 的流程生成完整脚本，输出格式遵循 `script-format.md`，虚实标注遵循 `annotation.md`。
